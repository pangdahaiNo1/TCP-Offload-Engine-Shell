...
#pragma HLS INLINE   off
  ...
  static ap_uint<16> connections_id_arr[TCP_MAX_SESSIONS];
#pragma HLS DEPENDENCE variable = cur_connection_id inter false
  ...
  switch (fsm_state) {
    ...
    case WAIT_USER_START:
      if (settings_regs.run_experiment && !run_experiment_reg) {  // Rising edge
        ...
        if ((settings_regs.num_connections > 0) &&
          ...
          packet_mss_reg            = settings_regs.packet_mss;  // Register input variables
    ...
    case WAIT_CON:
      if (!net_app_open_conn_rsp.empty()) {
        ...
        if (open_conn_rsp.data.success) {
          ...
          connections_id_arr[cur_connection_id] = open_conn_rsp.data.session_id;
          cur_connection_id++;
    ...
    case REQ_WRITE_FIRST:
      trans_data_req.id = connections_id_arr[cur_connection_id];
      if (use_timer_reg) {
      ...
      logger.Info(
          NET_APP, TX_APP_IF, "Trans data req for REQ_WRITE_FIRST", trans_data_req.to_string());
      cur_connection_id++;
    ...
    case SPACE_RESPONSE:
      if (!net_app_trans_data_rsp.empty()) {
        ...
        // cout << "SPACE_RESPONSE Response for transfer " << setw(5) << std::dec << sessionIt << "
        // length : " << spaceReplay.length; cout << "\tremaining space: " <<
        // spaceReplay.remaining_space << "\terror: " <<  spaceReplay.error << std::endl <<
        // std::endl;
    ...
    case CLOSE_CONN:
      ...
      break;
    case CLOSE_CONN1:
      close_conn_req.data = connections_id_arr[cur_connection_id];
...
#pragma HLS STREAM variable = stop_watch_end_fifo depth = 4
  ...
  IperfServerDummy(net_app_new_client_notification,
