...
#pragma HLS DEPENDENCE variable                                     = rx_sar_table inter false
  ...
  if (!tx_eng_to_rx_sar_lup_req.empty()) {
    ...
    logger.Info(TX_ENGINE, RX_SAR_TB, "Lup Req", session_id.to_string(16));
  ...
  // Read or Write access from the Rx App I/F to update the application pointer
    ...
    logger.Info(RX_APP_IF, RX_SAR_TB, "R/W Req", rx_app_req.to_string());
    if (rx_app_req.write) {
      ...
      logger.Info(RX_SAR_TB, RX_APP_IF, "Read Rsp", rx_app_rsp.to_string());
  ...
  // Read or Write access from the Rx Engine
    ...
    logger.Info(RX_ENGINE, RX_SAR_TB, "R/W Req", recv_new_seg_req.to_string());
